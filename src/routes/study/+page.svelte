<script>
  ...
  async function markKnown() {
    ...
    // 暗記済みになったらメッセージを表示して、3秒後に次のカードに移動する
    if (isNowMemorized) {
  ...
  const todayCompletedKey = `todayCompleted_stage1_${todayKey}`;
