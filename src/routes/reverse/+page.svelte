<script>
  ...
  onMount(async () => {
    try {
      ...
      // Supabaseから進捗読み込み
      const { data, error: sbError } = await supabase.from("word_status").select("word_no, status, is_favorite, is_pending, review_count, next_review_at").eq("stage", 2); // stage2（日本語→タイ語）のデータだけ取得
